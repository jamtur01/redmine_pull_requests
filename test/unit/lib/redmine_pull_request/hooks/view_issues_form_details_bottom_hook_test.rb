...
class RedminePullRequest::Hooks::ViewIssuesFormDetailsBottomHookTest < ActionController::TestCase
  ...
  context "#view_issues_form_details_bottom for a user with permission" do
    setup do
      ...
      @form.expects(:text_area).
        with(:pull_requests, {:cols => 60, :rows => 3}).
        returns("<p><textarea name='issue[pull_requests]'></textarea></p>")
    end

    should "display a text area for pull requests" do
      @response.body = hook :issue => @issue, :form => @form

      assert_select 'textarea[name=?]', 'issues[pull_requests]'
    end
