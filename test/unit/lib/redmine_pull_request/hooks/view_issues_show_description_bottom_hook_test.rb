...
class RedminePullRequest::Hooks::ViewIssuesShowDescriptionBottomTest < ActionController::TestCase
  ...
  context "#view_issues_show_description_bottom for a user with permission" do
    ...
    should "return a div for the urls" do
      ...
      assert_select 'div.pull-requests'
    end

    should "return the label for the section" do
      @response.body = hook(:issue => @issue)

      assert_select 'p strong', :text => 'Pull Requests'
    end

    context "for an issue with pull requests" do
      setup do
        @issue.update_attribute(:pull_requests,
                                "https://support.example.comi/ticket/123\n" +
                                "#456, #789\n" +
